...
// Add Zen Tabs styles
if (theme_get_setting('deadpool_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'deadpool') .'/css/tabs.css');
}

function deadpool_preprocess_html(&$vars) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . deadpool_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = deadpool_id_safe('page-'. $path);
    $vars['classes_array'][] = deadpool_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('deadpool_ie_enabled')) {
    $deadpool_ie_enabled_versions = theme_get_setting('deadpool_ie_enabled_versions');
    if (in_array('ie8', $deadpool_ie_enabled_versions, TRUE)) {
...
function deadpool_preprocess_page(&$vars, $hook) {
...
function deadpool_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from deadpool_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function deadpool_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function deadpool_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('deadpool_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('deadpool_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('deadpool_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('deadpool_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('deadpool_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function deadpool_id_safe($string) {
 ...
 *
 */
function deadpool_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = deadpool_id_safe($element['#title']);
  // Adding a class depending on the ID of the link (constant)
  if (isset($element['#original_link']['mlid']) && !empty($element['#original_link']['mlid'])) {
    $element['#attributes']['class'][] = 'mid-' . $element['#original_link']['mlid'] . ' level-' . $element['#original_link']['depth'];
  }
  if (!empty($sub_menu)) {
    $sub_menu = '<div class="sub-menu level-' . $element['#original_link']['depth'] . '"><div class="wp lvl-' . $element['#original_link']['depth'] . '">' . $sub_menu . '</div></div>';
  }
 ...
 */
function deadpool_preprocess_menu_local_task(&$variables) {
 ...
 */
function deadpool_menu_local_tasks(&$variables) {
  $output = '';
